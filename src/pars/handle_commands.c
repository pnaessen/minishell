...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/10 15:00:35 by vicperri          #+#    #+#             */
/*   Updated: 2025/04/10 15:00:37 by vicperri         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_operators(char *args, int i)
{
	...
	else if (ft_is_operator(args[i]) == SUCCESS)
	{
		if (ft_is_operator(args[i + 1]) == SUCCESS && args[i + 1] != args[i])
			return (SUCCESS);
	}
	return (ERROR);
}
